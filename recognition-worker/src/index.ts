...
async function recognizeHumming(audio: File, env: Env, requestId: string): Promise<RecognitionResponse> {
  ...
  return { success: true, confidence: Number.isFinite(score) ? Math.round(Math.max(0, Math.min(1, score)) * 100) : 0, song: { id: candidate.acrid || `${artist}:${title}`, title, artist, album: String(candidate.album?.name || "Unknown Album"), artworkUrl: null, durationMs: Number(candidate.duration_ms || 0), isrc: candidate.external_ids?.isrc || null, spotifyUrl: spotifyId ? `https://open.spotify.com/track/${spotifyId}` : null, appleMusicUrl: null, youtubeMusicUrl: youtubeId ? `https://music.youtube.com/watch?v=${youtubeId}` : `https://music.youtube.com/search?q=${encodeURIComponent(`${artist} ${title}`)}`, audiomackUrl: `https://audiomack.com/search?q=${encodeURIComponent(`${artist} ${title}`)}` } }
}
