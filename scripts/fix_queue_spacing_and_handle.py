...
# Remove row-level rounding. The queue rows themselves are now completely sharp/rectangular.
...
# The newer row revision may use a slightly different compact one-line callback form.
s = s.replace('detectDragGesturesAfterLongPress(', 'detectDragGestures(', 1)

# Slightly bring action icons toward the screen edge.
s = s.replace('.padding(horizontal = 24.dp)\n                        .size(24.dp)', '.padding(horizontal = 16.dp)\n                        .size(24.dp)', 1)

p.write_text(s, encoding='utf-8')
print('Applied compact sharp queue rows, separated drag handle lines, and immediate drag recognition.')
